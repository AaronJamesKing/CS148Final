...
void ImageWriter::SaveImage(int segNumber)
{
    ...
    string modifiedName = m_sFileName.substr(0, indx) + "_" + std::to_string(segNumber) + m_sFileName.substr(indx);
    
    indx = modifiedName.find_last_of(".");
    ...
    if (indx != string::npos) {
        ...
        string sub = modifiedName.substr(indx + 1);
        ...
        if (sub == "JPG" || sub == "JPEG")
            ...
            modifiedName = modifiedName.replace(indx + 1, sub.length(), "jpg");
        }
    }
    
    if (FreeImage_Save(fm, m_pOutBitmap, modifiedName.c_str(), 0)) {
